    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'search_report':
                ...
                subtotal = search.aggregate(r=Coalesce(Sum('subtotal'), 0,
                                                       output_field=FloatField())).get('r')
                iva = search.aggregate(r=Coalesce(Sum('iva'), 0,
                                                  output_field=FloatField())).get('r')
                total = search.aggregate(r=Coalesce(Sum('total'), 0,
                                                    output_field=FloatField())).get('r')
